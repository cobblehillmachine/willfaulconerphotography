	...
	var api = {
		...
		options = {
			...
			templates: {
				...
				tweet: '<p><a class="account" href="http://twitter.com/{{user.screen_name}}" title="{{user.name}} — {{user.description}}">@{{user.screen_name}}</a> - {{html}}</p><span class="meta"><time><a href="http://twitter.com/{{user.screen_name}}/statuses/{{id_str}}">{{time_ago}}</a></time></span>'
			}
		},
		ext = function(o1,o2){
			for(var key in o2){
